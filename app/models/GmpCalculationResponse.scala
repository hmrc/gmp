...
object ContributionsAndEarnings {
  ...
  def createFromNpsLcntearn(earnings: NpsLcntearn): ContributionsAndEarnings =
    ContributionsAndEarnings(
      earnings.rattd_tax_year,
      earnings.rattd_tax_year match {
...
object CalculationPeriod {
  ...
  def createFromNpsLgmpcalc(npsLgmpcalc: NpsLgmpcalc): CalculationPeriod =
    CalculationPeriod(
      npsLgmpcalc.scheme_mem_start_date.map(LocalDate.parse(_)),
      LocalDate.parse(npsLgmpcalc.scheme_end_date),
      f"${npsLgmpcalc.gmp_cod_allrate_tot}%1.2f",
      f"${npsLgmpcalc.gmp_cod_post_eightyeight_tot}%1.2f",
      npsLgmpcalc.revaluation_rate,
      npsLgmpcalc.gmp_error_code,
  ...
  def createFromHipGmpDetails(details: GuaranteedMinimumPensionDetails): CalculationPeriod =
    CalculationPeriod(
      ...
      revalued = Some(if details.revaluationCalculationSwitchIndicator then 1 else 0),
      ...
      inflationProofBeyondDod = Some(if details.inflationProofBeyondDateofDeath then 1 else 0),
...
object GmpCalculationResponse {
  ...
  def createFromCalculationResponse(calculationResponse: CalculationResponse)(
    ...
    GmpCalculationResponse(
      ...
      nino,
      scon,
      revaluationRate.map(_.toString),
